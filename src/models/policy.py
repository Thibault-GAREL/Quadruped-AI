...
class NeuroPolicy:
    ...
    def __init__(self, nn_config: Dict[str, Any]):
        ...
        # Genes de MORPHOLOGIE places EN TETE du genome (0 pour les animaux au
        # squelette fige). Ils ne sont pas des poids : c'est l'animal evolutif
        # qui les lit pour se construire un corps, le reseau ne voit que la
        # suite du vecteur. Les garder dans le meme genome permet a un seul GA
        # de faire evoluer le corps et le cerveau ensemble.
        self.morpho_genes = int(nn_config.get('morpho_genes', 0))
        ...
        self.num_params = self.mlp.num_params + self.morpho_genes

    def split_genome(self, genome: np.ndarray):
        """Separe (genes de morphologie, poids du reseau)."""
        if not self.morpho_genes:
            return None, genome
        return genome[:self.morpho_genes], genome[self.morpho_genes:]
    ...
    def act(self, time: float, dog_state: Dict[str, Any], genome: np.ndarray) -> np.ndarray:
        ...
        _, weights = self.split_genome(genome)
        return self.mlp.forward(self.build_input(time, dog_state), weights)
